...
- **Deploy command:** `ssh root@72.61.6.37` and run the commands in `.deploy/16-pull-and-rebuild.sh` (the file is local on Windows workstation; not present on VPS — paste contents over SSH). **CRLF gotcha:** the local `.deploy/*.sh` files are CRLF and break when run via `bash` on the VPS (`set: -`, `cd: /opt/apps/paperclip\r`, `not a git repository`). Strip CR first: `scp .deploy/16-pull-and-rebuild.sh root@72.61.6.37:/tmp/deploy16.sh` then `ssh root@72.61.6.37 "sed -i 's/\r//g' /tmp/deploy16.sh && bash /tmp/deploy16.sh"`. Migrations from an upstream sync apply on the **new container's boot** during this deploy (not via a manual pre-apply — the running container is the old image and lacks the new migration files); the pre-flight's job is backup + dry-run of any destructive migration BEFORE the deploy. See `reference_paperclip_migration_apply_on_boot` + `feedback_deploy_crlf_strip` memories.
